#Stored programme in github
#studentId:W2051753/20230716
#Date:2023/11/19
import sys #since i need to stop student once he add data! 
from graphics import *
class User:#introdusing class we can add multiple users(staff menbers or students) for this code
    ...
    def checkUser(self):#afer this i neet call getinput
        try: 
            who=str(input("If Youre Student Type  --> S \If Youre Staff Member  -->  M >:"))
            if who.lower() == "s":
                self.Callforstudents()         
            elif who.lower()== "m":
                self.Callforstaff()
            else:
                print("Sorry I Dont Get It :( Try Again !")
        except:
            pass
                   
    def Callforstudents(self):
        self.getInput()
        self.main()
        sys.exit("Bye.!") #student need once to check their result

    def Callforstaff(self):
        self.getInput()
        self.main()
